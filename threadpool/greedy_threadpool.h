...
// Forward declaration.
template<typename T> struct greedy_threadpool_receit;
...
struct greedy_threadpool
{
	...
	void worker(size_t me)
	{
		...
		while (!done)
		{
			...
			if (!done) // Done has priority. For example, if the condition was done:true && active::false, then honor done:false.
			{
				...
				grd_tp_info("I am thread(" + ::juwhan::to_string(me) + "). Someone ordered me to stop crunching. I will stop until further notice.");
	...
	: done{false}, joiner{threads}, master_queues{}, my_queue{}, neighboring_queues{}, master_neighboring_queues{}, mut{}, cond{}, active{false}
	{
...
template<typename T> struct greedy_threadpool_receit_base_implementation
{
...
template<typename T> struct greedy_threadpool_receit : public greedy_threadpool_receit_base_implementation<T>
{
...
template<> struct greedy_threadpool_receit<void> : public greedy_threadpool_receit_base_implementation<void>
{
