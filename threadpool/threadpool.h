...
// Forward declaration.
template<typename T> struct threadpool_receit;
...
struct threadpool
{
	...
	void worker(size_t me)
	{
		...
		while (!done)
		{
			...
			if (fetched_task)
			{
				...
				tp_info("I am about to execute the task. My queue had " + to_string(old_outstanding_count) + " outstanding tasks and now it has " + to_string(outstanding_count.load()) + ".");
				(*fetched_task)();
			...
			else
			{
				...
				lock.unlock();
				tp_info("I(" + to_string(me) + ") woke up. The wake up condition is outstanding_count: " + to_string(outstanding_count.load()) + ". I'll resume working.");
			}
		}
	...
	: done{false}, joiner{threads}, master_queues{}, my_queue{}, neighboring_queues{}, master_neighboring_queues{}, outstanding_count{0}, mut{}, cond{}
	{
	...
	submit(F&& _func, A&&... args)
	{
		...
		tp_info("Before submitting the task, my queue had " + to_string(old_outstanding_count) + " outstanding tasks and now it has " + to_string(outstanding_count.load()) + ".");
...
template<typename T> struct threadpool_receit_base_implementation
{
	...
	void wait()
	{
		...
		while (!ret.is_set() && !tp->done)
		{
			...
			if(fetched_task)
			{
				...
				tp_info("I am about to execute the task. My queue had " + to_string(old_outstanding_count) + " outstanding tasks and now it has " + to_string(tp->outstanding_count.load()) + ".");
				(*fetched_task)();
...
template<typename T> struct threadpool_receit : public threadpool_receit_base_implementation<T>
{
...
template<> struct threadpool_receit<void> : public threadpool_receit_base_implementation<void>
{
