...
template<typename T> struct return_type
{
  ...
  return_type& operator=(T& _value) { value = _value; state = return_state::valid; return *this; };
  return_type& operator=(T&& _value) { value = ::juwhan::move(_value); state = return_state::valid; return *this; };
  return_type& operator=(return_state _state) { state = _state; return *this; };
};
...
public:
  ...
  work_stealing_queue(size_t requested_size) : array{new array_type{requested_size}}, top{1}, bottom{1} 
  {
    ...
    if(init_result) throw ::std::runtime_error("An unknown error occurred while trying to initialize a rwlock in a work stealing queue: error code " + to_string(init_result) + ".");
  };
  work_stealing_queue() 
  ...
  size_t size() { auto b = bottom.load(::std::memory_order_relaxed); auto t = top.load(::std::memory_order_relaxed); return (b>t)?b-t:0;};
...
*/
    ...
    if(lock_result) throw ::std::runtime_error("An unknown error occurred while trying to acquire a write lock while growing the array size: error code " + to_string(lock_result) + ".");
    array.store(new_a, ::std::memory_order_seq_cst);
    auto release_result = pthread_rwlock_unlock(&rwlock);
    if(release_result) throw ::std::runtime_error("An unknown error occurred while trying to release a write lock while growing the array size: error code " + to_string(lock_result) + ".");
  ...
  void push(T x)
  {
    ...
    if (b > t + a->size - 1) 
    {
        wsq_info("While trying to push an element at the bottom index " + to_string(b) + ", array overflow occured. Corresponding top index was " + to_string(t) + ".");
        grow(t, b, a);
  ...
  return_type<T> pop()
  {
    ...
    if (t<=b)
    {
      ...
      if (t==b)
      {
        ...
        {
          wsq_info("While attempting to pop the last element from the queue, the element disappeared. Perhaps another thread stole it.");
          // Failed in race.
    ...
    wsq_info_if(x, "I popped an item successfully. Now, the top index is " + to_string(top.load()) + " and the bottom index is " + to_string(bottom.load()) + ".");
    return x;
  };
  ...
  return_type<T> steal() 
  {
    ...
    wsq_info("Stealing an element at the top index " + to_string(t) + ", where the bottom index is " + to_string(b));
    ...
    if (t < b) 
    {
      ...
      if(lock_result) throw ::std::runtime_error("An unknown error occurred while trying to acquire a read lock while stealing an item: error code " + to_string(lock_result) + ".");
      auto a = array.load(::std::memory_order_consume);
      x = a->get(t);
      auto release_result = pthread_rwlock_unlock(&rwlock);
      if(release_result) throw ::std::runtime_error("An unknown error occurred while trying to release a read lock while stealing an item: error code " + to_string(lock_result) + ".");
      if (!top.compare_exchange_strong(t, t + 1, ::std::memory_order_seq_cst, ::std::memory_order_relaxed))
      {
        // Failed in race.
        wsq_info("While attempting to steal an element from the queue, the element disappeared. Perhaps another thread stole it or it was the last element and was popped. Now, the top index is " + to_string(top.load()) + " and the bottom index is " + to_string(bottom.load()) + ".");
        return return_type<T>{T{}, return_state::abort};
      }
    }
    wsq_info_if(x, "I stole an item successfully. Now, the top index is " + to_string(top.load()) + " and the bottom index is " + to_string(bottom.load()) + ".");
    return x;
  };
};
