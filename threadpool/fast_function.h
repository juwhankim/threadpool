...
// A function pointer type to any global static functions
        ...
        template<class TOut, class TIn>
        inline TOut horrible_cast(TIn mIn)
    ...
    template<class TOut, class TIn>
    inline TOut unsafe_horrible_cast(TIn mIn)
...
struct simplify_member_function {
    ...
    inline static ptr_to_any_class convert(const TThis *, TFunc, ptr_to_void_void_member_func &)
...
struct simplify_member_function<size_of_a_ptr_to_void_void_member_func> {
    template<class TThis, class TFunc>
    inline static ptr_to_any_class convert(const TThis *mThis, TFunc mFunc, ptr_to_void_void_member_func &mFuncOut)
...
public:
    template<class TThis, class TFunc>
    inline void bind(TThis *mThis, TFunc mFunc)
    ...
    inline void bind(TThis *mThis, TInvoker mInvoker, ptr_to_static_func mFunc)
    ...
    inline bool operator==(ptr_to_static_func mPtr) const
    ...
    noexcept { return !operator==(mRhs); }

    inline bool operator!=(ptr_to_static_func mPtr) const

    noexcept { return !operator==(mPtr); }
    ...
    inline ptr_to_any_class get_this_ptr() const

    noexcept    { return this_ptr; }

    inline ptr_to_member_func get_member_func_ptr() const

    noexcept    { return reinterpret_cast<ptr_to_member_func>(func_ptr); }

    inline ptr_to_static_func get_static_func_ptr() const

    noexcept { return horrible_cast<ptr_to_static_func>(this); }
};
...
public:
    inline fast_function_implementation()

    noexcept =
    default;

    inline fast_function_implementation(::std::nullptr_t)

    noexcept {}

    inline fast_function_implementation(ptr_to_static_func mFunc)

    noexcept { bind(mFunc); }

    template<typename X, typename Y>
    inline fast_function_implementation(X *mThis, Y mFunc)

    noexcept { bind(mThis, mFunc); }

    inline fast_function_implementation &operator=(ptr_to_static_func mFunc)
    ...
    noexcept    { return closure == nullptr; }

    inline bool operator==(const fast_function_implementation &mImpl) const

    noexcept { return closure == mImpl.closure; }

    inline bool operator==(ptr_to_static_func mFuncPtr) const

    noexcept { return closure == mFuncPtr; }

    inline bool operator!=(::std::nullptr_t) const

    noexcept { return !operator==(nullptr); }

    inline bool operator!=(const fast_function_implementation &mImpl) const

    noexcept { return !operator==(mImpl); }

    inline bool operator!=(ptr_to_static_func mFuncPtr) const

    noexcept { return !operator==(mFuncPtr); }
...
template<typename T>
struct member_func_to_static_func;
