...
template<typename T> struct function_return_type
{
	...
	void set(T& value) { *value_ptr = value; base->set(); };
	void set(T&& value) { *value_ptr = move(value); base->set(); };
...
template<typename T> struct function_return_type<T&>
{
	T** value_ptr;
	function_return_type_base_implementation* base;
	function_return_type() : base{new function_return_type_base_implementation}, value_ptr{new T*} {};
...
template<> struct function_return_type<void>
{
	function_return_type_base_implementation* base;
...
// Thread_task helper.
template<size_t N, typename H, typename... T> struct thread_task_helper {};
...
T_T_H_MACRO(1); T_T_H_MACRO(2); T_T_H_MACRO(3); T_T_H_MACRO(4); T_T_H_MACRO(5); T_T_H_MACRO(6); T_T_H_MACRO(7); T_T_H_MACRO(8); T_T_H_MACRO(9); T_T_H_MACRO(10);
#undef T_T_H_MACRO
...
struct thread_task_implementation : thread_task_helper<1, A...>, public thread_task
{
	...
	template<typename FF, typename... AA, typename = typename enable_if<function_type_deduction<typename decay<FF>::type>::is_static || function_type_deduction<typename decay<FF>::type>::is_functor>::type> 
	...
	:  thread_task_helper<1, A...>(::juwhan::forward<AA>(args)...), func(::juwhan::forward<T>(this_), ::juwhan::forward<FF>(func_)) {};
...
EXE_MACRO(1); EXE_MACRO(2); EXE_MACRO(3); EXE_MACRO(4); EXE_MACRO(5); EXE_MACRO(6); EXE_MACRO(7); EXE_MACRO(8); EXE_MACRO(9); EXE_MACRO(10);
#undef EXE_MACRO 
...
struct thread_task_implementation<F> : public thread_task
{
	...
	// 1. The static and the functor case.
	template<typename FF, typename = typename enable_if<function_type_deduction<typename decay<FF>::type>::is_static || function_type_deduction<typename decay<FF>::type>::is_functor>::type>
...
typename enable_if<function_type_deduction<typename decay<F>::type>::is_static || function_type_deduction<typename decay<F>::type>::is_functor, thread_task*>::type
make_task(F&& func_, A&&... args)
{
	// Use malloc to void* free.
	auto thread_task_pointer = malloc(sizeof(thread_task_implementation<typename decay<F>::type, typename decay<A>::type...>));
	new (thread_task_pointer) thread_task_implementation<typename decay<F>::type, typename decay<A>::type...>{::juwhan::forward<F>(func_), ::juwhan::forward<A>(args)...};
...
make_task(F&& func_, T&& this_, A&&... args)
{
	// Use malloc to void* free.
	auto thread_task_pointer = malloc(sizeof(thread_task_implementation<typename decay<F>::type, typename decay<A>::type...>));
	new (thread_task_pointer) thread_task_implementation<typename decay<F>::type, typename decay<A>::type...>{::juwhan::forward<F>(func_), ::juwhan::forward<T>(this_), ::juwhan::forward<A>(args)...};
	return reinterpret_cast<thread_task*>(thread_task_pointer);
};
