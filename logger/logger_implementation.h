...
class delimit
{
	...
	string right_delim;
public:
	delimit(const string& left_in={}, const string& right_in={}) : left_delim{left_in}, right_delim{right_in} {};
...
class colorize
{
	...
	template<typename T, typename... Types> void recursive_init(const T value, const Types... args)
	{
		...
		static_assert(is_same<ANSI_foreground_color, T>::value 
			...
			|| is_same<ANSI_text_attribute, T>::value, "Input type T to recursive_init<typename T...> must be either ANSI_foreground_color, ANSI_background_color, ANSI_text_attribute, or delimit.");
	...
	template<typename... Types> void recursive_init(const delimit value, const Types... args)
	{
		// We don't need to check the type of value since it is already specialized to delimit.
		delim=value;
		recursive_init(args...);
	}
	template<typename T> void recursive_init(const T value)
	{
		// Peel off the last and only argument, and check if it is a proper one.
		static_assert(is_same<ANSI_foreground_color, T>::value 
			|| is_same<ANSI_background_color, T>::value
			|| is_same<ANSI_text_attribute, T>::value, "Input type T to colorize<typename T...> must be either ANSI_foreground_color, ANSI_background_color, or ANSI_text_attribute.");
		// Now, initialize according to the type.
		if (is_same<ANSI_foreground_color, T>::value)
		{
			foreground_color = static_cast<unsigned int>(value);
...
public:
	...
	template<typename T, typename... Types> colorize(const T value, const Types... args)
	...
	template<typename T, typename... Types> void configure(const T value, const Types... args)
	{
		recursive_init(value, args...);
	}
	template<typename T, typename... Types> void reset(const T value, const Types... args)
	{
		foreground_color = 0;
		background_color = 0;
	...
	virtual string info() const
	{
		...
		stream << "This colorization object has the foreground color code: " << foreground_color << ", the background color code: " << background_color;
		if(!text_attributes.empty()) 
		{
			stream << ", and the formatting codes: {" << *text_attributes.begin();
			for(auto iter=text_attributes.begin()+1; iter!=text_attributes.end();iter++) stream << ", " << *iter;
			stream << "}";
		}
		stream << "." << endl;
		...
		if(foreground_color)
		{
			...
			if(background_color)
			{
				...
				if(!text_attributes.empty())
				{
					...
					for(auto attr=text_attributes.begin()+1;attr!=text_attributes.end();attr++) stream << ";" << *attr;
				}
			}
		}
...
struct locate : locate<Target, Tail...> { static constexpr unsigned int value = locate<Target, Tail...>::value + 1; };

template<typename Target, typename... Tail>
struct locate<Target, Target, Tail...> { static constexpr unsigned int value = 0;};
...
class log_formatter : public log_formatter<Tail...>
{
	void is_valid()
	{
		...
		static_assert(is_base_of<formatting_element_with_message, Head>::value
			|| is_base_of<formatting_element_without_message, Head>::value, "Bad types to log_formatter");
	}
...
public:
	...
	log_formatter(const Candidate c, const The_rest... tr) // This function did not hit the Head type, so keep calling the base constructor until a hit is made.
	...
	template<typename... Ms> string compose(const Ms&... ms) const
	{
	...
	template<typename Target, typename... Arg> void configure(Arg... arg)
	{
		get<Target>().configure(arg...);
	}
	// A utility helper to get and reset elements at once.
	template<typename Target, typename... Arg> void reset(Arg... arg)
	{
...
public:
	...
	: element{h} { is_valid(); };
	// Compose a log message.
	template<typename M, typename... Ms> string compose(const M& m, const Ms&... ms) const
	{
	...
	get()
	{
		static_assert(true, "Something bad happened. Either you tried to find a non-existing element in log_formatter or simply it's a bug.");
	}
	// A utility helper to get and configure elements at once.
	template<typename Target, typename... Arg> void configure(Arg... arg)
	{
...
public:
	...
	head_element() = delete;
	template<typename... T> head_element(const string& name, T... args) : head{name}, base{args...} {};
	// Inherit configure from colorize.
	operator string() const
	{
...
public:
	...
	// Order of elemental initialization must be kept, although some can be omitted.
	template<typename Candidate, typename... The_rest>
	log_line(const int i, const Candidate c, const The_rest... tr) // This function did not hit the Head type, so keep calling the base constructor until a hit is made.
	: priority{i}, base{c, tr...}, element{} { is_valid(); };
	template<typename... The_rest>
	log_line<The_rest...>(const int i, Head h, The_rest... tr) // We hit the correct type. Peel it off from the pack.
	...
	// Operator ().
	template<typename... Ms> string operator()(const Ms&... ms) const
	{
		return compose(ms...);
	}
};
